...
def parse_mstock_response(data):
    ...
    try:
        ...
        change = int(change_str.replace(',', '').replace('+', ''))
        ...
        change_rate = float(rate_str.replace(',', '').replace('+', '').replace('%', ''))
...
def get_stock_price(code):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if result:
                print(f"[Price OK - polling] {code}: {result['price']:,}원")
                return result
    except Exception as e:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if result:
                print(f"[Price OK - m.stock] {code}: {result['price']:,}원")
                return result
    except Exception as e:
    ...
    try:
        ...
        if r.status_code == 200:
            result = parse_mstock_response(r.json())
            if result:
                print(f"[Price OK - api.stock] {code}: {result['price']:,}원")
                return result
    except Exception as e:
...
def api_debug(code):
    """디버그: 각 API별 응답 확인 (문제 진단용)"""
    debug = {}
    endpoints = [
        ('polling', f'https://polling.finance.naver.com/api/realtime?query=SERVICE_ITEM:{code}'),
        ('m_stock', f'https://m.stock.naver.com/api/stock/{code}/basic'),
        ('api_stock', f'https://api.stock.naver.com/stock/{code}/basic'),
        ('ac_search', f'https://ac.stock.naver.com/ac?q={code}&target=stock'),
    ]
